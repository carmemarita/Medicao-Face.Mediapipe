...
def calcular_ini_fim_linha_diagonal(ponto1, ponto2, largura):
    m = (ponto1[1] - ponto2[1]) / (ponto1[0] - ponto2[0])  # Inclinação
    b = ponto1[1] - m * ponto1[0]  # Interceptação no eixo y
    # Traçando a linha estendida por toda a largura da imagem
    x_inicio = 0
    y_inicio = int(m * x_inicio + b)
    x_fim = largura
    y_fim = int(m * x_fim + b)
    return ((x_inicio, y_inicio), (x_fim, y_fim))

# Função para calcular o ângulo da linha em relação ao eixo horizontal
def calcular_angulo(ponto1, ponto2):
    delta_x = ponto2[0] - ponto1[0]
    delta_y = ponto2[1] - ponto1[1]
    angulo_rad = math.atan2(delta_y, delta_x)  # Calcula o ângulo em radianos
    angulo_graus = math.degrees(angulo_rad)   # Converte para graus
    return angulo_graus

# Função para traçar uma linha em um dado ângulo, passando por um ponto específico
def calcular_ini_fim_linha_horiz(ponto, angulo, largura):
    
    # Calculando o deslocamento para estender a linha
    comprimento_linha = largura  # A linha será estendida para a largura da imagem
    angulo_rad = math.radians(angulo)

    # Calculando os pontos finais da linha com base no ângulo
    x_inicio = int(ponto[0] - comprimento_linha * math.cos(angulo_rad))
    y_inicio = int(ponto[1] - comprimento_linha * math.sin(angulo_rad))
    x_fim = int(ponto[0] + comprimento_linha * math.cos(angulo_rad))
    y_fim = int(ponto[1] + comprimento_linha * math.sin(angulo_rad))
...
PONTA_QUEIXO_DIR = 378
PONTA_ORELHA_DIR = 401
PONTA_SOMB_DIR = 383
PONTA_QUEIXO_ESQ = 149
PONTA_ORELHA_ESQ = 177
PONTA_SOMB_ESQ = 156
...
for arquivo in Path(PATH_PASTA).iterdir():
    ...
    lst_pontos_estudo.append( Ponto("PONTA_QUEIXO_DIR", PONTA_QUEIXO_DIR, "", "", "", 0) )
    lst_pontos_estudo.append( Ponto("PONTA_ORELHA_DIR", PONTA_ORELHA_DIR, "", "", "", 0) )
    lst_pontos_estudo.append( Ponto("PONTA_SOMB_DIR", PONTA_SOMB_DIR, "", "", "", 0) )
    lst_pontos_estudo.append( Ponto("PONTA_QUEIXO_ESQ", PONTA_QUEIXO_ESQ, "", "", "", 0) )
    lst_pontos_estudo.append( Ponto("PONTA_ORELHA_ESQ", PONTA_ORELHA_ESQ, "", "", "", 0) )
    lst_pontos_estudo.append( Ponto("PONTA_SOMB_ESQ", PONTA_SOMB_ESQ, "", "", "", 0) )
        ...
        if (extensao == ".jpg"):
                ...
                if results.multi_face_landmarks:
                    for face_landmarks in results.multi_face_landmarks:
                        ...
                        dir_ponta_queixo = face_landmarks.landmark[PONTA_QUEIXO_DIR]
                        dir_ponta_orelha = face_landmarks.landmark[PONTA_ORELHA_DIR]
                        dir_ponta_sombrancelha = face_landmarks.landmark[PONTA_SOMB_DIR]
                        esq_ponta_queixo = face_landmarks.landmark[PONTA_QUEIXO_ESQ]
                        esq_ponta_orelha = face_landmarks.landmark[PONTA_ORELHA_ESQ]
                        esq_ponta_sombrancelha = face_landmarks.landmark[PONTA_SOMB_ESQ]
                        ...
                        x_dir_ponta_queixo, y_dir_ponta_queixo = int(dir_ponta_queixo.x * largura), int(dir_ponta_queixo.y * altura)
                        x_dir_ponta_orelha, y_dir_ponta_orelha = int(dir_ponta_orelha.x * largura), int(dir_ponta_orelha.y * altura)
                        x_dir_ponta_sombrancelha, y_dir_ponta_sombrancelha = int(dir_ponta_sombrancelha.x * largura), int(dir_ponta_sombrancelha.y * altura)
                        x_esq_ponta_queixo, y_esq_ponta_queixo = int(esq_ponta_queixo.x * largura), int(esq_ponta_queixo.y * altura)
                        x_esq_ponta_orelha, y_esq_ponta_orelha = int(esq_ponta_orelha.x * largura), int(esq_ponta_orelha.y * altura)
                        x_esq_ponta_sombrancelha, y_esq_ponta_sombrancelha = int(esq_ponta_sombrancelha.x * largura), int(esq_ponta_sombrancelha.y * altura)
                        ...
                        ###########################################
                        # ESCREVE LINHAS HORIZONTAIS

                        angulo = calcular_angulo((x_eye_esquerdo_interno, y_eye_esquerdo_interno), 
                                                 (x_eye_direito_interno, y_eye_direito_interno))

                        #######
                        (inicio,fim) = calcular_ini_fim_linha_horiz((x_mid_eyes, y_mid_eyes), angulo, largura)
                        cv2.line(image, (inicio[0], inicio[1]), (fim[0], fim[1]), (194, 163, 157), TAM_LINHA)

                        #######
                        (inicio,fim) = calcular_ini_fim_linha_horiz((x_base_do_nariz, y_base_do_nariz), angulo, largura)
                        cv2.line(image, (inicio[0], inicio[1]), (fim[0], fim[1]), (194, 163, 157), TAM_LINHA)

                        #####
                        (inicio,fim) = calcular_ini_fim_linha_horiz((x_queixo, y_queixo), angulo, largura)
                        cv2.line(image, (inicio[0], inicio[1]), (fim[0], fim[1]), (194, 163, 157), TAM_LINHA)

                        ###########################################
                        # ESCREVE LINHAS DIAGONAIS 

                        #####
                        (inicio,fim) = calcular_ini_fim_linha_diagonal((x_mid_eyes, y_mid_eyes), 
                                                                       (x_queixo, y_queixo), largura)
                        cv2.line(image, (inicio[0], inicio[1]), (fim[0], fim[1]), (194, 163, 157), TAM_LINHA)

                        #####
                        (inicio,fim) = calcular_ini_fim_linha_diagonal((x_dir_ponta_queixo, y_dir_ponta_queixo), 
                                                                       (x_dir_ponta_orelha, y_dir_ponta_orelha), largura)
                        cv2.line(image, (inicio[0], inicio[1]), (fim[0], fim[1]), (43, 43, 255), TAM_LINHA)

                        #####
                        (inicio,fim) = calcular_ini_fim_linha_diagonal((x_esq_ponta_queixo, y_esq_ponta_queixo), 
                                                                       (x_esq_ponta_orelha, y_esq_ponta_orelha), largura)
                        cv2.line(image, (inicio[0], inicio[1]), (fim[0], fim[1]), (43, 43, 255), TAM_LINHA)

                        #####
                        (inicio,fim) = calcular_ini_fim_linha_diagonal((x_esq_ponta_queixo, y_esq_ponta_queixo), 
                                                                       (x_esq_ponta_sombrancelha, y_esq_ponta_sombrancelha), largura)
                        cv2.line(image, (inicio[0], inicio[1]), (fim[0], fim[1]), (43, 43, 255), TAM_LINHA)

                        #####
                        (inicio,fim) = calcular_ini_fim_linha_diagonal((x_dir_ponta_queixo, y_dir_ponta_queixo), 
                                                                       (x_dir_ponta_sombrancelha, y_dir_ponta_sombrancelha), largura)
                        cv2.line(image, (inicio[0], inicio[1]), (fim[0], fim[1]), (43, 43, 255), TAM_LINHA)
